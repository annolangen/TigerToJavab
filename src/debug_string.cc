...
std::ostream& AppendDebugString(const syntax::Declaration& d, std::ostream& out,
    DebugStringOptions options) {
  std::visit(syntax::Overloaded{[&](const syntax::FunctionDeclaration& f) {
    out << "function " << f.id << "(";
    const char* sep = "";
...
std::ostream& AppendDebugString(
  ...
  std::visit(syntax::Overloaded{[&](const syntax::StringConstant& c) {
    out << "\"" << c.value << "\"";
    ...
    out << "if " << Str{*i.condition, options} << " then "
        << Str{*i.then_expr, options} << " else " << Str{*i.else_expr, options};
  },
...
std::ostream& AppendDebugString(
    const syntax::LValue& v, std::ostream& out, DebugStringOptions options) {
  std::visit(
...
std::ostream& AppendDebugString(
    const syntax::Type& t, std::ostream& out, DebugStringOptions) {
  std::visit(
      syntax::Overloaded{[&](const syntax::Identifier& id) { out << id; },
