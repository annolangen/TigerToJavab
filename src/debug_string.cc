...
std::ostream& AppendDebugString(const syntax::Declaration& d, std::ostream& out, DebugStringOptions options) {
  std::visit(syntax::Overloaded{
                 [&](const syntax::FunctionDeclaration& f) {
                   ...
                   out << "): " << f.type_id.value_or("NOTYPE") << std::endl << options.indent;
                 ...
                 [&](const syntax::VariableDeclaration& v) {
                   out << "var " << v.id << ": " << v.type_id.value_or("NOTYPE") << " := " << Str{*v.value, options};
                 },
                 [&](const syntax::TypeDeclaration& t) { out << "type " << t.id << " = " << Str{t.value, options}; }},
             d);
...
std::ostream& AppendDebugString(const syntax::Expr& e, std::ostream& out, DebugStringOptions options) {
  std::visit(
      syntax::Overloaded{
          [&](const syntax::StringConstant& c) { out << "\"" << c.value << "\""; },
          [&](const syntax::IntegerConstant& c) { out << c; }, [&](const syntax::Nil&) { out << "nil"; },
          [&](const std::unique_ptr<syntax::LValue>& l_value) { AppendDebugString(*l_value, out, options); },
          [&](const syntax::Negated& n) { out << "-" << Str{*n.expr, options}; },
          [&](const syntax::Binary& b) {
            out << Str{*b.left, options} << " " << b.op << " " << Str{*b.right, options};
          },
          [&](const syntax::Assignment& a) { out << Str{*a.l_value, options} << " := " << Str{*a.expr, options}; },
          ...
          [&](const syntax::ArrayLiteral& a) {
            out << a.type_id << " [" << Str{*a.size, options} << "] of " << Str{*a.value, options};
          },
          [&](const syntax::IfThen& i) {
            out << "if " << Str{*i.condition, options} << " then " << Str{*i.then_expr, options};
          },
          [&](const syntax::IfThenElse& i) {
            out << "if " << Str{*i.condition, options} << " then " << Str{*i.then_expr, options} << " else "
                << Str{*i.else_expr, options};
          },
          [&](const syntax::While& w) {
            out << "while (" << Str{*w.condition, options} << ") " << Str{*w.body, options};
          },
          [&](const syntax::For& f) {
            out << "for " << f.id << " := " << Str{*f.start, options} << " to " << Str{*f.end, options} << " do "
                << Str{*f.body, options};
          },
          ...
          [&](const syntax::Let& l) {
            ...
            for (const auto& expr : l.body) {
              out << body_options.indent << Str{*expr, body_options} << std::endl;
            }
            out << "end";
          },
...
std::ostream& AppendDebugString(const syntax::LValue& v, std::ostream& out, DebugStringOptions options) {
  std::visit(syntax::Overloaded{[&](const syntax::Identifier& id) { out << id; },
                                [&](const syntax::RecordField& f) { out << Str{*f.l_value, options} << "." << f.id; },
...
std::ostream& AppendDebugString(const syntax::Type& t, std::ostream& out, DebugStringOptions) {
  std::visit(syntax::Overloaded{[&](const syntax::Identifier& id) { out << id; },
                                ...
                                [&](const syntax::ArrayType& a) { out << "array of " << a.element_type_id; }},
             t);
  return out;
}
